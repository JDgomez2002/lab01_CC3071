...
def directConstruction(regex):
    Dstates = []
    Dtransitions = []
    tree = SyntaxTree(regex)
    language = tree.operands
    node_map = tree.render()
    Dstates.append(DFAState(calc_firstpos(tree.root), False, False, True)) # store root first pos
                    ...
                    for endNode in newState:
                        if isinstance(endNode, int):
                            if nodeValueAndFollowpos[endNode-1][2] == {'Ø'}:
                                acceptingState = True
                                break
                    Dstates.append(DFAState(newState, False, acceptingState)) # add the new state to Dstates
                if currentState.state != {'Ø'}: # if the current state is not empty
                    Dtransitions.append(DFATransition(symbol, currentState.state, newState)) # add the transition to Dtransitions
...
states, transitions = directConstruction("(a|b)*abb#")
# initialStates = []
# acceptingStates = []
# for state in states:
#     if state.initial:
...
create_direct_dfa_graph(states, transitions)
